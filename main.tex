...
\begin{abstract}
Companies are increasingly employing sophisticated machine learning models, which can be highly sensitive to data error.
Training data are often integrated from a variety of sources and are susceptible to inconsistencies--biases which propagate to predictions.
Automatically detecting and repairing such data is in general very challenging, but we leverage the key insight that we can often query a relatively clean test dataset to evaluate the improvement in predication accuracy of a proposed cleaning operations.
We present a system called \sys that utilizes this insight by optimizing over a set of possible conditional data cleaning operations (detector-repair tuples) by casting the problem as statistical boosting.
\sys provides a library of dataset-independent error detectors
based on deterministic heuristics and statistical criteria. 
To better detect errors in categorical attributes, one of the modules in this library is a novel anomaly detector based on the Word2Vec Neural Network architecture.
Once the errors are detected, \sys ``hedges its bets'' by selected an ensemble of models trained with different repair strategies for the detected error.
We evaluated \sys on a collection of datasets from machine learning competitions, real-world data analyses, and datasets from \company, and found up-to 9\% improvements in absolute prediction accuracy. 
